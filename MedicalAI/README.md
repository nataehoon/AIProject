...
MedicalAI는 DICOMDIR가 포함된 의료 영상 ZIP 파일에서 환자 메타데이터와 영상 프레임을 추출하고, Vision Language Model(VLM)이 생성한 판독 초안을 의료 Q&A 및 논문 데이터에 기반한 Retrieval-Augmented Generation(RAG)으로 보강한 뒤, Large Language Model(LLM)을 통해 최종 보고서 형태로 정리하는 애플리케이션입니다.
...
| 계층 | 주요 파일 | 역할 |
| --- | --- | --- |
| Presentation | `home.py`, `view/` | 페이지 라우팅, 로그인, DICOM 업로드, 분석 진행 상태와 결과 표시 |
| Service | `services/` | 인증, 의료 데이터 저장, RAG 데이터 동기화·버전 관리, 검색 흐름 조정 |
...
├── services/
│   ├── chat_service.py             # 의료 채팅 서비스(개발 중)
...
| 의료 채팅 | 개발 중 | `ChatService` 라우팅·응답 로직 미완성 |
| 자동 DB migration | 미구현 | ERD만 포함 |
| 자동화 테스트 | 미구현 | `practice/`는 실험용 스크립트 |
...
- [ ] 의료 채팅 라우팅 및 환자 분석 기록 연동 완성
- [ ] 단위·통합·E2E 테스트와 CI 파이프라인 구축
- [ ] 모델 품질 평가 지표와 임상 검토 워크플로 추가

## 기여 방법

이슈나 Pull Request를 만들기 전에 변경 범위와 의료 데이터 취급 여부를 확인해 주세요.

1. 저장소를 Fork하고 기능 브랜치를 생성합니다.
2. 변경 사항과 함께 필요한 테스트 또는 재현 절차를 추가합니다.
3. 민감 정보와 실제 환자 데이터가 포함되지 않았는지 확인합니다.
4. 변경 목적, 구현 방법, 검증 결과를 Pull Request에 작성합니다.

## License

현재 저장소에는 라이선스 파일이 포함되어 있지 않습니다. 외부 공개, 배포 또는 재사용 전에 프로젝트 라이선스와 사용 중인 모델·데이터셋의 개별 라이선스를 확인하고 명시해야 합니다.
